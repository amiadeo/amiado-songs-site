...
const COVERS = {
  ...
  'ltzat-lchvpshy':          `<img src="songs45.jpeg" alt="לצאת לחופשי"          style="width:100%;height:100%;object-fit:cover;display:block">`,
};
...
const SONGS = [
  {
    id: 'ltzat-lchvpshy', title: 'לצאת לחופשי', language: 'he',
    sunoEmbedId: 'd6139d60-dc92-4c3a-9ba1-dd29b74df05b',
    chords: [],
    lyrics: [
      { section: 'בית 1', lines: [
        'לו הייתי שערה על ראשך',
    ...
    analysis: {
      abstract: 'השיר "לצאת לחופשי" בוחן את המתח שבין מחויבות ושליטה לבין הכמיהה לשחרור מוחלט. דרך דימוי אינטימי של שערה, הדובר מביע רצון להשיל מעליו את רעשי העולם המודרני – ההישגיות, התחרות וה"חשבונות" – לטובת חזרה למצב ראשוני של מנוחה, שקט וחיבור לטבע. זהו שיר על הצורך האנושי "להתפזר" כדי למצוא את עצמך מחדש.',
      structural: 'הניגודיות בין האיסוף לפיזור: המבנה נשען על המעבר מהבית הראשון, המציג "איסוף" (קוקו מתוח, שליטה), אל הפזמון והבית השני המקדשים את ה"פיזור" (שיער פזור, רוח, חול). מוטיב המירוץ: החזרה על השלילה בפזמון ("לא לדפוק חשבון", "לא לרדוף") יוצרת תחושה של מרידה בקצב החיים הסטנדרטי. סגירה מעגלית: השיר מסתיים בדעיכה של המילה "חופשי", מה שמעניק למבנה אופי של אנחת רווחה מתמשכת.',
      psychological: 'הפרדוקס של האוטונומיה: הדובר מבקש להיות "שערה על ראשך" – מצב של ביטול עצמי לכאורה בתוך האחר, אך בפועל זהו מרחב המוגן מהציפיות החברתיות. האבן מהלב: הדימוי הפסיכולוגי של שחרור ה"אבן" מעיד על עומס רגשי מצטבר. הצורך ברגרסיה חיובית: הבקשה "רק קצת לישון" על החול מסמלת שיבה למצב עוברי, נטול אחריות, שבו האדם אינו נדרש להיות "במקום הראשון" כדי להרגיש ראוי.',
      academic: 'השיר מתכתב עם תפיסות אקזיסטנציאליסטיות של "חופש מ-" (Freedom from). הוא מגדיר את החופש לא כעשייה, אלא כהיעדר מוחלט של תפקוד תחת פיקוח חברתי. השימוש בשפה יומיומית ("לדפוק חשבון") לצד מטפורות ליריות קלאסיות (הרוח והים) מייצר "סינסטזיה רגשית" – שבה המופשט (חופש) מקבל ביטוי פיזי מוחשי (חול חם, שיער). השיר דן בשחיקת הזהות האינדיבידואלית בתוך המרחב הציבורי ההישגי.'
    }
  },
  {
  ...
  },
  {
    ...
    analysis: {
      ...
      psychological: 'משבר הזהות: \'לחפש בי ולראות בי משהו אחר\' — הדובר מרגיש שהמבט החיצוני עליו מעוות את דמותו. הסכמים והשלמות: \'הסכמי חיבה ולא כוונה\' — תיאור חריף של זיוף חברתי שאין בו סליחה.',
    ...
    <div class="page-enter song-page${song.language === 'en' ? ' lang-en' : ''}"${song.language === 'en' ? ' dir="ltr"' : ''}>
      ...
      <div class="song-hero">
        ...
        <div class="song-hero-mobile-actions">
          ...
          <button class="shma-btn track-add-btn" data-song-id="${song.id}" title="הוסף לפלייליסט">
          ...
          <button class="shma-btn btn-share-song" data-song-id="${song.id}" data-song-title="${song.title}" title="שתף">
